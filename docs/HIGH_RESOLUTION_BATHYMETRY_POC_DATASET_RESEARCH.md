...
## 12. Post-MVP-055: 海しるAPI `等深線` 公式問い合わせ準備記録

対象Issue: #153 / Post-MVP-055
記録日: 2026-07-16

### 12.1 公式問い合わせ窓口

海しるAPI開発者ポータルの利用方法ページで、利用登録による個別サブスクリプションキー発行は「海上保安庁海洋情報部問い合わせフォーム」から受け付けると案内されている。問い合わせ内容には、キーワードで「海洋状況表示システム（海しる）」を選択し、件名と内容に利用目的や取得したい海しるAPIの情報項目等をできる限り詳細に記載する手順が示されている。

- 公式ページ: https://portal.msil.go.jp/howtouse
- 問い合わせフォーム: https://www1.kaiho.mlit.go.jp/joint/form_mail/form_mail.cgi
- 問い合わせ手段: Webフォーム
- キーワード: `海洋状況表示システム（海しる）`
- 件名案: `海しるAPI「等深線」の仕様および派生物Web配信可否の確認`

### 12.2 送信状況

未送信。Codex実行環境では、利用者本人の氏名、メールアドレス、電話番号等の入力と送信確認を伴う公式Webフォーム送信を代理実行しない。したがって、本Issueの成果は送信前Markdown記録、送信先、送信手順、送信予定文面の整備までとし、送信済みとは扱わない。

送信手順:

1. 公式問い合わせフォームを開く: https://www1.kaiho.mlit.go.jp/joint/form_mail/form_mail.cgi
2. 必須項目の氏名、E-mail、キーワード、海域、件名、内容を入力する。
3. キーワードで `海洋状況表示システム（海しる）` を選択する。
4. 海域で `第七管区` を選択する。
5. 件名に `海しるAPI「等深線」の仕様および派生物Web配信可否の確認` を入力する。
6. 内容欄に下記文面を貼り付け、利用者本人の所属・連絡先・利用目的に合わせて必要箇所を補う。
7. 入力内容を確認し、利用者本人が送信する。
8. 送信日、送信者、受付番号または自動返信の有無を本ドキュメントへ追記する。
...
海洋状況表示システム（海しる）APIの利用を検討している個人開発プロジェクトです。釣り向けの参考用Web地図で、水深・3D地形表示の高精細化PoCを行う前に、海しるAPI「等深線」の仕様および加工・派生物公開条件を公式に確認したく問い合わせいたします。

本問い合わせは、以下の暫定boundsに限定したPoC可否判断を目的とします。このboundsは問い合わせ用の暫定値であり、公式coverageまたは採用boundsとは扱いません。
...
確認したい項目は以下です。回答可能な範囲で、該当する公式文書、API仕様、メタデータ、利用条件ページをご案内いただけますと幸いです。

## 1. データ識別・仕様

- 「等深線」のendpoint、layer ID、product ID等の一意識別子
- 属性schemaおよび主要フィールド
- 元となる水深グリッドの正式名称、dataset ID、product ID
- 上記暫定boundsでのcoverageおよび公式bounds
- native grid spacingまたは等深線間隔
- 水平座標系
- 鉛直基準／datum
- 水深単位
- nodata定義
...
- 必要なpaging回数
- 正式な取得手順
- APIキー、申請、利用登録、アクセス制限
- 再取得時に同一結果を確認するためのmetadataまたは更新日情報

## 3. 加工・派生物・Web配信

上記boundsに限定したPoCとして、海しるAPI「等深線」または取得可能な元データを利用し、以下を行えるか確認したいです。

- 元データの加工・変換
- Terrain-RGBへの変換
- PNGタイルの生成
- GeoJSON等深線の生成・加工
- 派生物をGitHub repositoryへ格納すること
- 派生物をVercel Previewまたは公開Webアプリで配信すること
- 派生物の再配布
- 第三者権利、外部DB条件、個別制約の有無
- attribution要件
- 加工表示および非保証文言
- 商用／非商用条件
- 書面許可、申請、追加問い合わせの要否

本PoCでは、高頻度アクセス、スクレイピング、元データにない地形生成、航海・安全判断への利用は行いません。水深表示は参考情報として扱い、必要な出典表示、加工表示、非保証表示を実装する前提です。

上記のうち、回答が難しい項目や別窓口へ確認すべき項目があれば、適切な確認先をご教示ください。よろしくお願いいたします。
```

### 12.4 回答状況・確認事項・未確認事項

- 回答状況: 未送信のため未回答。
- 回答で確認できた項目: なし。
- 未確認事項: endpoint/layer ID/product ID、属性schema、元グリッド正式名称、coverage、公式bounds、native grid spacingまたは等深線間隔、水平座標系、鉛直基準、単位、nodata、対象bounds総レコード数、総容量、paging回数、正式取得手順、再取得metadata、加工・変換可否、Terrain-RGB/PNG/GeoJSON派生物生成可否、GitHub/VercelでのWeb配信・再配布可否、第三者権利、attribution、加工表示、非保証文言、商用／非商用条件、書面許可要否。

### 12.5 Phase B開始可否

Phase Bは **開始不可 / No-Go継続**。未送信・未回答であり、判断ゲートのうち、具体的dataset/product/layerの一意特定、対象boundsでの切り出し可否、GEBCO_2026 15秒より有意に細かい比較可能性、取得量、加工・派生物生成、GitHub/Vercel等でのWeb配信・再配布許可が未確認である。公式回答または公式文書で全ゲートを満たすまで、高精細データ取得、Terrain-RGB/PNG/GeoJSON生成、Vercel配置は開始しない。
