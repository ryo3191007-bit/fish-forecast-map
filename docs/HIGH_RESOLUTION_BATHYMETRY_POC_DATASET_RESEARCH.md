...
- 問い合わせフォーム: https://www1.kaiho.mlit.go.jp/KIKAKU/toiawase.html
- 問い合わせ手段: Webフォーム
...
1. 公式問い合わせフォームを開く: https://www1.kaiho.mlit.go.jp/KIKAKU/toiawase.html
2. 必須項目の氏名、E-mail、キーワード、件名、内容を入力する。
3. キーワードで `海洋状況表示システム（海しる）` を選択する。
4. 件名に `海しるAPI「等深線」の仕様および派生物Web配信可否の確認` を入力する。
5. 内容欄に下記文面を貼り付け、利用者本人の所属・連絡先・利用目的に合わせて必要箇所を補う。
6. 入力内容を確認し、利用者本人が送信する。
7. 送信日、送信者、受付番号または自動返信の有無を本ドキュメントへ追記する。
