...
- 生態JSONあり: **70件** / なし: **10件**
- schemaVersion: `1.2.0` 1件 / `1.3.0` 20件 / `1.4.0` 49件
- v1.4移行: **49/67 taxon** / 未移行または未調査claimあり: **18 taxon**
- v1.4 researchState（identity 2 + ecology 16 claim）: `not_researched` 0 / `complete` 881 / `blocked` 1 / `not_applicable` 0
- v1.4 unknownReason: `not_researched` 0 / `source_not_found` 251 / `insufficient_evidence` 44 / `conflicting_evidence` 0 / `scope_mismatch` 317 / `taxonomy_uncertain` 1
- 既存JSON内の `stableGeneral` unknown: **354/560属性 (63.2%)**
  - species_group: **96/96属性 (100.0%)**
  - 個別taxon（species_group以外）: **258/464属性 (55.6%)**
- 既存JSON内の `regionalCatchability` unknown: **560/560属性 (100.0%)**
- JSON未作成を8属性未評価として含めた `stableGeneral` 未解決相当: **434/640属性 (67.8%)**
- `stableGeneral` 8/8 unknown: **17件**
