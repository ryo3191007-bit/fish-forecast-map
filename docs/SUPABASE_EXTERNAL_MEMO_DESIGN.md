...
- `local-data-not-migrated` 状態では、明示的な移行操作が入るまで登録・編集・削除もlocalStorageへ固定する。操作をきっかけにlocalStorage由来メモを暗黙にDBへ移さない。
