...
1. `supabase/sql/005_external_catch_memos_owner_policies.sql` をSupabase SQL Editorで手動実行してよいかのチェックポイントを設ける。
2. SQL手動実行後、Dashboardまたは安全な検証手順でauthenticated owner scoped RLSが期待通りか確認する。
3. 外部メモrepositoryをログイン中user id前提のDB read/writeへ接続する。
4. UIからDB read/writeを任意接続し、失敗時はlocalStorage fallbackを維持する。
5. 明示的な移行操作を設計し、重複登録と論理削除方針を確認する。
...
Post-MVP-024の範囲では、DB write、RLS policy追加、UI保存先のDB切替は行わない。Post-MVP-025でRLS policy SQL案のみを追加するが、SQL Editor実行、実DB反映、repositoryのDB write有効化は行わない。ログイン後も外部メモは既存のlocalStorage保存のままとし、Auth設定手順は `docs/SUPABASE_AUTH_SETUP.md` を参照する。
