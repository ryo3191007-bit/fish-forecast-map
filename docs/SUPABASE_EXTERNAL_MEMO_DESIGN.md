...
修正後の削除repositoryは、削除後行のRETURNING/SELECTに依存しない。まず `id`、`owner_id = user.id`、`is_deleted = false` で削除前の対象行が存在することを確認し、存在しない場合は対象行0件または別ユーザー行として成功扱いにしない。その後、同じ `id` / `owner_id` / `is_deleted = false` 条件で `is_deleted = true` へ論理削除し、RETURNINGは要求せず、更新件数が1件である場合だけDB削除成功と判定する。

この方針により、owner scoped RLSと通常SELECTの `is_deleted = false` 制約は維持しつつ、DB削除成功時はlocal tombstoneを作らず保存先表示をSupabaseのまま維持する。本当にDBエラー、対象行0件、別ユーザー行、競合更新などで削除できなかった場合だけ、従来どおりユーザー別local tombstone fallbackで画面上の非表示を維持する。

## RLS対応RPCによる論理削除（Post-MVP-030 / Issue #85）

Post-MVP-030では、外部釣果メモの削除をクライアントからの直接 `update` ではなく、RLS対応RPC `public.soft_delete_external_catch_memo(p_memo_id text)` 呼び出しへ変更する。SQLファイルは `supabase/sql/006_soft_delete_external_catch_memo_rpc.sql` に追加し、Supabase Dashboard / SQL Editorでの実行はユーザーが手動で行う。

RPCの仕様は次の通り。

- 対象は `authenticated` のみで、`anon` / `public` には実行権限を付けない。
- `search_path` を `public, auth` に固定する。
- `id = p_memo_id`、`owner_id = auth.uid()`、`is_deleted = false` の行だけを対象にする。
- 対象行には `is_deleted = true` と `updated_at = now()` だけを設定する。
- 更新行そのものは `RETURNING` せず、`GET DIAGNOSTICS ... ROW_COUNT` で更新件数を取得する。
- 更新件数が1件のときだけ `true`、0件や別ユーザー行、既に削除済みの行は `false` を返す。
- service role keyは使わず、owner scoped RLS、通常SELECTで `is_deleted = true` 行を読ませない方針、物理deleteなしの方針を維持する。

repositoryは `.rpc("soft_delete_external_catch_memo", { p_memo_id: memoId })` を呼び、戻り値が `true` の場合だけDB削除成功と判定する。`false`、RPCエラー、Supabase未設定、未ログインは成功扱いにせず、従来どおりlocal tombstone fallbackで画面上の非表示を維持する。RPC成功時はlocal tombstoneを作らず、Supabase利用可能状態を維持する。

削除失敗の診断として、repositoryが返す `result.meta.message` はURL、JWT、長いキーらしき値、UUID、`owner_id` 条件値を伏せた安全な文字列に正規化する。通常ユーザー向けUI文言は従来どおり簡潔にし、開発時だけ `console.warn` で `external-catch-memo:delete` の安全な診断メッセージを確認できるようにする。本番ビルドではこの診断ログは出力しない。

### 手動SQL適用手順

1. Supabase Dashboardで対象プロジェクトを開く。
2. SQL Editorを開く。
3. リポジトリの `supabase/sql/006_soft_delete_external_catch_memo_rpc.sql` の内容を確認して貼り付ける。
4. DB URL、DB password、JWT、実キー、`.env.local` の内容を貼り付けていないことを確認する。
5. SQLを実行する。
6. エラーが出た場合は、その画面に秘密情報が含まれていないことを確認したうえで、エラー概要だけを共有して再確認する。

### 適用後の本番再確認手順

1. 本番アプリへログインする。
2. 外部メモの保存先表示が `Supabase` または `Supabase + localStorage` としてDB利用可能であることを確認する。
3. 新規外部メモを登録し、Supabase Dashboardで `external_catch_memos` に自分の未削除行が作成されることを確認する。
4. そのDB由来メモを画面から削除する。
5. 通常UIにDB削除失敗のfallbackメッセージが出ないこと、保存先表示がSupabase利用可能状態のままであることを確認する。
6. Supabase Dashboardで対象行の `is_deleted` が `true`、`updated_at` が更新済みであることを確認する。
7. ブラウザを再読込し、削除したメモが復活しないことを確認する。
8. 必要に応じてlocalStorage由来メモも削除し、localStorage由来メモはDB RPCではなくブラウザ保存側だけで削除されることを確認する。
