...
`006` 適用後の本番確認で、RLSの `WITH CHECK` と通常SELECT policyの影響により `42501` が継続したため、追加適用SQL `supabase/sql/007_harden_soft_delete_external_catch_memo_rpc.sql` で同じRPCを最小権限の `security definer`、`set search_path = ''` として再定義する。関数内では `auth.uid()` を取得し、NULLなら `false` を返す。更新対象は `id = p_memo_id`、`owner_id = auth.uid()` 相当、`created_by = 'authenticated_user'`、`is_deleted = false` をすべて満たす所有・未削除行だけに限定するため、`admin_import` / `manual_local_storage_migration` 由来の行はRPC削除対象にしない。更新内容は `is_deleted = true` と `updated_at = pg_catalog.now()` のみで、1件更新できた場合だけ `true` を返す。`public` / `anon` から実行権限を剥奪して `authenticated` にだけ `execute` を付与する。service roleは使わず、既存のowner scoped RLS、`owner_id = auth.uid()`、`created_by = 'authenticated_user'`、通常SELECTの `is_deleted = false` 制約、物理delete禁止を維持する。
...
2. `supabase/sql/006_soft_delete_external_catch_memo_rpc.sql` を未適用の場合は内容を確認して実行する。
3. 追加hardening SQL `supabase/sql/007_harden_soft_delete_external_catch_memo_rpc.sql` の内容を確認して実行する。
4. 本番アプリでログインし、DB由来の外部メモを新規作成する。
5. そのメモを削除し、画面の保存先表示がSupabase利用可能状態のままであること、通常UIに生DBエラーが出ないことを確認する。
6. Supabase Dashboardで対象行の `is_deleted = true` を確認する。
7. 画面を再読込し、対象メモが復活しないことを確認する。
8. 失敗時だけ、ブラウザコンソールにサニタイズ済みの短い診断が出ることを確認する。実URL、JWT、キー、`owner_id`、ユーザーUUIDはログへ貼り付けない。

## 明示的localStorage移行導線（Post-MVP-031 / Issue #90）

ログイン済みでSupabase readが成功し、現在ユーザーに紐づくlocalStorage由来の手動外部釣果メモが残っている場合だけ、外部メモモーダル内に控えめな移行導線を表示する。自動移行やログイン直後の暗黙移行は行わず、ユーザーがチェックボックスで選択したメモだけを現在ログイン中ユーザーのSupabase領域へ1件ずつ保存する。

安全方針は次の通り。

- 既存localStorage key `fish-forecast-map.external-catch-memos` は変更しない。
- 移行対象は現在ユーザーに表示可能なlocalStorage由来かつ `acquisitionMethod: "manual"` のメモだけに限定する。
- DB由来メモ、別ユーザー用fallback、別ユーザー用tombstoneは移行候補へ混入させない。
- 同一IDがDB由来メモとして既に見えている場合は、無条件上書きせずスキップする。
- Supabase保存は配列全件upsertではなく、選択されたメモを1件ずつ `insert` する。
- DB payloadは既存repository方針どおり `owner_id = auth.uid()` 相当、`created_by = 'authenticated_user'`、`is_deleted = false` を維持する。
- DB保存前にlocalStorageから削除しない。
- DB保存が成功しても、Supabaseから再取得して対象IDを確認できるまでlocalStorageから削除しない。
- DB保存失敗、再取得確認失敗、重複、未ログイン、DB利用不可など判断できないケースではlocalStorageを保持する。
- 一部成功・一部失敗の場合は、成功かつ再取得確認済みの対象だけをlocalStorageから外し、失敗・未選択メモはlocalStorageに残す。
- 移行結果として成功、スキップ、失敗件数を表示し、未移行メモが残ることを明示する。

本番手動確認では、ログイン後に既存localStorageメモがあるブラウザで外部釣果メモ登録モーダルを開き、候補件数、選択UI、移行先が現在ユーザーのSupabaseである説明、自動移行ではない説明、確認完了までlocalStorageを消さない説明が表示されることを確認する。1件だけ選択して移行し、再読込後にそのメモがSupabase由来として重複なく表示され、未選択メモはlocalStorage由来として残ることを確認する。
