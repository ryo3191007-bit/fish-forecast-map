...
| ID | 地点名 | 地域区分 | 緯度 | 経度 |
|---|---|---|---:|---:|
| `ouka-port` | 相賀漁港 | 唐津湾沿岸 | 33.506447 | 129.956971 |
| `kodomo-port` | 小友漁港 | 呼子・鎮西 | 33.546840 | 129.906629 |
| `kabeshima-port` | 加部島漁港 | 呼子・鎮西 | 33.556550 | 129.889400 |
| `hado-port` | 波戸漁港 | 呼子・鎮西 | 33.546300 | 129.859000 |
| `haregi-port` | 晴気漁港 | 肥前・玄海沿岸 | 33.435100 | 129.811000 |
| `tobo-port` | 唐房漁港 | 唐津湾沿岸 | 33.482000 | 129.940000 |
| `minatohama-port` | 湊浜漁港 | 唐津湾沿岸 | 33.526093 | 129.954970 |
| `nagoya-port` | 名護屋漁港 | 呼子・鎮西 | 33.532610 | 129.877582 |
| `yobuko-port` | 呼子漁港 | 呼子・鎮西 | 33.545850 | 129.890760 |
| `takakushi-port` | 高串漁港 | 肥前・玄海沿岸 | 33.422500 | 129.826000 |
...
5. `historicalTargetSpecies` は新規参考項目として保持し、設備候補は汎用 `facilities` を作らず既存の `toilet` / `lighting` / `parking` / `access` へ分割する。民間由来情報は `weak_evidence / low` とし、SCOREへ入力しない。
6. 公的資料が直接確認する漁港の存在・種別と、民間参考情報による波止、テトラ、岸壁、港内構造、頭上電線を同じ値へ混在させない。具体構造は `coastal_topography` / `obstacles` 等へ分離して `weak_evidence / low` とする。
7. 公的根拠は `has_evidence`、民間の地点固有参考情報は `weak_evidence`、調査しても現在の状況を確定できないものは `researched_unknown` とする。
8. 規制情報を設備・利用可能情報より優先して表示する。
9. 新規10地点は全てJMA警報区域の佐賀県唐津市へマッピングする。
10. migrationはforward-only。既存データを削除または上書きしない。
11. DB取得失敗時も静的fallbackで同じ10地点を利用できるようにする。
