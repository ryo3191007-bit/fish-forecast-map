...
4. public schemaに想定外の差分がないことを確認する
5. `.remote-history-synced` を追加するPRを作成する

本番で手動適用した関数の改行コードがローカルmigrationと異なる場合、`db diff`が同一内容の関数再定義だけを出力することがあります。`scripts/check-bootstrap-schema-diff.mjs` は、`soft_delete_external_catch_memo` の署名、`SECURITY DEFINER`、空の `search_path`、所有者条件、`created_by` 条件、論理削除条件を完全一致で検証し、改行差だけを許容します。別の関数、追加SQL、条件変更が含まれる場合は停止します。
