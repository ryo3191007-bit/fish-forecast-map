...
このIssueでは設計のみを扱い、Supabaseプロジェクト作成、クライアント導入、環境変数追加、DBマイグレーション、SQL実装、API Route追加、UI変更、localStorage移行実装は行いません。
...
- Supabaseを導入する場合でも、最初は読み取り専用マスターのDB化など、認証なしで安全に扱える範囲から始めます。
...
7. 認証を導入し、ユーザー所有データとRLSを有効化する。
8. ユーザー自身の釣果登録・釣果日記を設計、実装する。
9. SCORE履歴や地点評価履歴の保存が必要か検証し、必要な場合だけスナップショットテーブルを追加する。
10. 公開範囲拡大時の座標丸め、危険地点の扱い、利用規約整備を行う。
