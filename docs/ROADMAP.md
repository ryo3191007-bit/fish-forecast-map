...
- 気象庁情報を環境タブで確認でき、取得失敗を「安全」と扱わない。
- 取得失敗を「安全」と扱わない。
...
- 本人実地調査はcurated researchや安全・規制情報を上書きしない。
...
## Phase 8: FishTraceへのリブランド

Issue #393で主要導線を「釣り場を探す・現地調査する・記録する・共有する」へ統一します。

- 主要UI、ブラウザタイトル、metadataを `FishTrace` へ統一。
- `地点評価` を `地点情報` へ変更。
- SCORE、釣れやすさ予測、魚種・釣法おすすめを主要UIから撤去。
- 地図、地点情報、環境情報、釣果記録、ユーザー地点、本人実地調査、認証は維持。
- 旧SCORE v2内部コードの全面削除はIssue #400で扱う。
