...
Post-MVP-001ではローカルのモック釣果から決定的にスコアを算出していましたが、Post-MVP-085で同梱モック釣果とそのSCORE補正経路をアプリ実行コードから削除しました。現行の地点評価は地点情報、環境データ、登録済み釣果を根拠とし、情報不足を架空の釣果で補完しません。

## 将来のAPI/データ方針

承認済みのIssueがない限り、有料APIや自動取り込みは追加しません。

外部釣果メモを `SCORE` へ反映する場合の前提、重み付け候補、時間帯・潮汐・天候・海況の一致度、根拠表示、安全側の制約は `docs/EXTERNAL_MEMO_SCORE_RULES.md` を正本として扱います。外部メモ単体は個別 `SCORE未計算` のまま扱います。Post-MVP-085以降、モック釣果SCOREへの補正経路は存在せず、地点評価では登録済み釣果をSCORE v2の実データ根拠として扱います。


## Post-MVP-009後の一覧UIとSCOREの役割分担

Post-MVP-009後は、釣果一覧と地点評価一覧の役割を分けます。

- 釣果一覧は、個別釣果データの日付、エリア、場所、魚種、釣果数、サイズ、釣り方、出典を確認する画面です。個別釣果の `SCORE` やスコア根拠は表示しません。
- 釣果一覧の絞り込みは、魚種、エリア、キーワード、開始日/終了日の期間フィルタを中心にします。現在UIにないデータ種別、外部メモ情報元、信頼度、釣り場紐づけフィルタは、現行仕様として扱いません。
- 釣果一覧の並び替えは日付系を中心にします。条件リセット時に `scoreDesc` へ戻す前提にはしません。
- 地点評価一覧は、地点ごとの平均 `SCORE` を扱う画面です。条件に合う手動外部メモがある場合は、既存地点の平均 `SCORE` に小さく参考反映された結果を扱います。
- Open-Meteo Weather / Marine APIの環境データは参考表示として扱い、公式潮汐表、安全判断、航行判断の代替にはしません。
...
- `standard`: 国土地理院の淡色地図 `pale` をMapLibreの `raster` source / `raster` layerとしてベースマップに表示します。タイルURLは `https://cyberjapandata.gsi.go.jp/xyz/pale/{z}/{x}/{y}.png`、`tileSize: 256`、`maxzoom: 18` とし、出典は `出典: 国土地理院ウェブサイト / 地理院タイル` を表示します。
