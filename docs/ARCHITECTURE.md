...
外部釣果メモを `SCORE` へ反映する場合の前提、重み付け候補、時間帯・潮汐・天候・海況の一致度、根拠表示、安全側の制約は `docs/EXTERNAL_MEMO_SCORE_RULES.md` を正本として扱います。Post-MVP-008以降、外部メモ単体は個別 `SCORE未計算` のまま扱い、条件に合う手動外部メモだけを既存のモック釣果/地点評価 `SCORE` へ小さく参考反映します。
