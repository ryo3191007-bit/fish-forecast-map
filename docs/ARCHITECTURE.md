...
外部釣果メモを将来 `SCORE` へ反映する場合の前提、重み付け候補、時間帯・潮汐・天候・海況の一致度、根拠表示、安全側の制約は `docs/EXTERNAL_MEMO_SCORE_RULES.md` を正本として扱います。この設計はドキュメント上の将来方針であり、現時点では外部メモは `SCORE未計算 / SCORE未反映` です。
