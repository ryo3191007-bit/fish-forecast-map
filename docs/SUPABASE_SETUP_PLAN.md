...
このドキュメントでは、Supabaseプロジェクト作成、DB作成、DBマイグレーション、SQL実装、API Route追加、UI変更、localStorage key変更、SCORE計算変更は行いません。Post-MVP-016で `@supabase/supabase-js` とクライアント初期化層を最小導入済みですが、本格的なDBテーブル、画面連携はまだ未実装です。読み取り専用の最小疎通確認だけは `docs/SUPABASE_READONLY_CONNECTION_CHECK.md` に分離しています。
...
- 読み取り専用接続の最小確認手順は `docs/SUPABASE_READONLY_CONNECTION_CHECK.md` を参照します。

## 外部釣果メモDB保存の準備段階

Post-MVP-023では、外部釣果メモ向けの `external_catch_memos` テーブルSQL案とmapper/repository土台を追加していますが、これはレビュー用の準備段階です。現時点では次を行いません。

- Supabase SQL Editorで `supabase/sql/004_external_catch_memos.sql` を実行すること。
- 実DBテーブル作成や実DBへの外部メモ投入。
- UI保存先のDB完全切替。
- 既存localStorage key変更や自動移行。
- 認証なしの `anon` write policy追加。

設計詳細は `docs/SUPABASE_EXTERNAL_MEMO_DESIGN.md` を参照します。安全確認はDB接続なしで `npm run check:external-memo-db` を実行します。
...
4. 読み取り専用接続の最小確認を行う（手順: `docs/SUPABASE_READONLY_CONNECTION_CHECK.md`、SQL: `supabase/sql/001_readonly_connection_check.sql`）。
5. `fish_species` / `fishing_spots` / `source_registry` のDB化設計を作る。
6. マスターデータ3テーブルのSQL定義を `supabase/sql/002_master_data_tables.sql` に追加済み。seed SQLは `supabase/sql/003_master_data_seed.sql`、手動実行手順は `docs/SUPABASE_MASTER_DATA_SETUP.md`、差分確認は `npm run check:master-seed` で確認する。ただし、まだSupabase SQL Editorでは未実行で、実DBテーブル作成、実DBへのseed投入、アプリ画面連携は未実装。次の候補は手動実行後のDB read + 静的データfallback実装。
...
- 実DBテーブル作成、画面連携。
- 読み取り専用接続の最小確認を超えるDB連携。
- 実キー、実URL、実パスワードの追加。
- `.env.local` の追加。
- DBマイグレーション実行。
- `supabase/sql/002_master_data_tables.sql` / `supabase/sql/003_master_data_seed.sql` のSQL Editor実行。
- API Route追加。
- UI変更。
- localStorage key変更。
- SCORE計算変更。
...
## Supabase migration運用（Issue #92）

今後のDBスキーマ変更の正本は `supabase/migrations/<timestamp>_<description>.sql` です。`supabase/sql/*.sql` は、過去にSQL Editorで手動適用済みの可能性がある参照資料として扱い、同じSQLを本番へ無条件に再実行しません。

### 初回にユーザーが一度だけ行うGitHub設定

値はGitHub画面へ直接入力し、チャット、Issue、PR、リポジトリへ貼り付けません。

1. Repository Settingsを開きます。
2. Environmentsで `production` を作成します。
3. Deployment branchesを `main` のみに制限します。
4. Environment secretsに次を登録します。
   - `SUPABASE_ACCESS_TOKEN`
   - `SUPABASE_DB_PASSWORD`
   - `SUPABASE_PROJECT_ID`
5. 通常の非破壊migrationはRequired reviewerなしで自動適用可能にします。破壊的変更はPRマージ前にユーザー承認を取ります。

### 初回remote履歴bootstrap

初回remote履歴同期が完了するまで、production workflowは `supabase/migrations/.remote-history-synced` が存在しないため安全に失敗し、`supabase db push` を実行しません。

bootstrapはユーザーとChatGPTが1手順ずつ確認して進めます。

1. `supabase migration list` 相当でremote履歴を確認します。
2. remote schemaとリポジトリのSQL/migration候補の差分を確認します。
3. 既にSQL Editorで適用済みの `supabase/sql/*.sql` を再実行しない前提で、必要ならbaseline migration化または履歴同期方針を決めます。
4. `supabase migration repair` が必要な場合も、実スキーマとmigration内容を照合してから実行します。repairはmigration履歴を修復する操作であり、スキーマ変更そのものではありません。
5. 不一致が残る場合はproduction deployを止めたままにします。
6. 履歴同期が完了したPRでだけ `supabase/migrations/.remote-history-synced` を追加し、以後のproduction deployを有効化します。

### CI / workflowの安全方針

- PRではproduction secretsを使わず、ローカルSupabaseの空DBへmigrationを順番に適用します。
- `main` へのpushで `supabase/migrations/**` が変わった場合だけproduction deploy workflowが起動します。app-only変更では起動しません。
- production workflowはGitHub Environment `production`、最小permissions、concurrencyを使います。
- Secrets未設定、履歴未同期、不整合、危険SQL検出時は失敗し、productionへ何も適用しません。
- production workflowはseed投入、DB reset、service role key利用、秘密情報ログ出力を行いません。

### 破壊的変更とレビュー必須事項

`npm run check:migration-safety` は `DROP TABLE`、`DROP COLUMN`、`TRUNCATE`、条件なしDELETE/UPDATE、RLS無効化、anon write、`GRANT ALL`、危険なsecurity definer候補を検出してCIを止めます。ただし単純な正規表現は安全保証ではありません。PRではRLS、権限、データ損失、復旧方針、アプリとの適用順序を必ずレビューします。

DB変更はforward-onlyを基本にし、後方互換性が必要な場合は expand → migrate → contract の段階適用を検討します。破壊的変更はユーザー承認前にマージしません。
