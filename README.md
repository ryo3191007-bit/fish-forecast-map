...
釣果予測マップは、福岡県糸島市西岸から唐津湾、伊万里湾、平戸方面までの陸っぱり釣り情報を地図で確認するための個人利用Webアプリです。堤防・磯場などでルアー釣り／エサ釣りを検討するときに、モック釣果と手動登録した外部釣果メモ、魚種、エリア、釣り方、釣れそう度スコアを一覧できます。
...
- 魚種フィルタ、エリアフィルタ、キーワード検索、データ種別、外部メモ情報元、信頼度、釣り場紐づけ、期間で絞り込みできます。
- 釣れそう度が高い順、日付が新しい順、日付が古い順で並び替えできます。
- 条件リセットボタンで、魚種 `all`、エリア `all`、キーワード空、並び順 `scoreDesc` に戻せます。
- モック釣果では0〜100点の説明可能な簡易ルールベースの釣れそう度スコアと根拠を確認できます。
- 手動URL登録による外部釣果メモを一覧・フィルタ・地図で確認できます。外部メモは現時点ではSCORE未計算・SCORE未反映です。
...
- 潮汐・天気・水温データ連携。
- 3D海底地形表示。
- 外部釣果メモのSCORE反映、実績ベースの高度な予測、複雑な機械学習。
- ページネーション、URLクエリ同期。
...
MVP v0.1の基本データはモック釣果データです。Post-MVP-005 / 006以降は、外部サイトを自動収集せず、ユーザーが手動でURLと必要最小限の事実情報を登録する外部釣果メモも表示対象にしています。外部メモは現時点ではSCORE未計算・SCORE未反映です。釣果情報には出典名と出典URLのフィールドを持たせますが、第三者サイト本文の転載、画像保存、コメント全文保存、プロフィール詳細保存、自動収集は行いません。
