...
FishTraceは、福岡県糸島市西岸から唐津湾、伊万里湾、平戸方面までを対象に、**釣り場を探す・現地調査する・釣果を記録する・共有する**ための個人利用Webアプリです。地図、地点情報、客観的な環境情報、ユーザー自身の釣果記録を一つの導線で確認できます。
...
- 地点情報は `環境 / 釣場 / 地形 / 魚種` の4タブで確認できます。
- 釣場・地形タブは通常表示地点で項目構成を共通化し、確認できない値も隠さず `未確定` と表示します。
- 魚種タブでは、その地点で自分が登録した釣果魚種と事前調査情報を分離して確認できます。
- ログイン中は、釣場・地形・魚種について自分が現地で確認した情報を追加・編集・削除できます。本人情報は調査済み値とは別レイヤーで保持し、安全・規制情報を上書きしません。
...
- SCORE・釣れやすさ予測・魚種や釣法のおすすめを主要導線として表示すること。
- 旧SCORE v2内部コードの全面削除（後続Issueで依存関係を確認して対応）。
...
MVP v0.1で使用した同梱モック釣果は、Post-MVP-085でアプリ実行コードから削除しました。地図には釣り場地点と地点へ紐づくユーザー自身の釣果記録を表示し、釣果一覧は本人の手入力記録を中心に扱います。実データがない場合に架空の釣果で補完しません。
...
魚種生態では、一般生態 `stableGeneral` と対象地域の陸っぱり釣れやすさ `regionalCatchability` を分離します。一般生態、他地域、船釣り、商業漁業等から地域の釣れやすさを推測しません。旧SCORE v2は主要UIから切り離し、内部コードの整理は後続Issueで行います。
...
次工程は、安全な環境からのAPI疎通確認と、実際に取得可能なデータ・エンドポイント・利用条件・出典条件の確認です。確認前に海しるデータを本番3D表示へ接続しません。
...
4. FishTraceの探索・現地調査・記録・共有導線を段階的に改善する。
5. Issue #195の残作業・close可否を整理する。
