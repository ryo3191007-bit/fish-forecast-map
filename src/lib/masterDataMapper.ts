import { fishSpeciesNames, type FishSpecies, type FishSpeciesName, type SpeciesCategory, type FishingMethod } from "@/domain/fishing";
...
export function mapFishSpeciesRow(row: FishSpeciesRow): FishSpecies | null {
  if (row.is_active === false || !speciesNameSet.has(row.name_ja)) return null;
