...
import { shouldUseGenericPhotoFilePicker } from "@/domain/photoFilePicker";
...
export function RecordPhotoEditor({ targetType, targetId, enabled, pending, onPendingChange, editable = true }: { targetType: RecordPhotoTargetType; targetId?: string; enabled: boolean; pending?: PreparedRecordPhoto[]; onPendingChange?: (photos: PreparedRecordPhoto[]) => void; editable?: boolean }) {
  ...
  const openPhotoPicker = () => {
    const useGenericPicker = typeof navigator !== "undefined" && shouldUseGenericPhotoFilePicker(navigator);
    (useGenericPicker ? filesFallbackInput : input).current?.click();
  };
  ...
  return <div className={styles.editor}>
    ...
    {editable && saved.length + selected.length < RECORD_PHOTO_LIMIT ? <><input ref={input} className={styles.input} type="file" accept="image/*" multiple disabled={photoDisabled || selected.length > 0} onChange={(event) => void choose(event.target.files)} /><input ref={filesFallbackInput} className={styles.input} type="file" accept={RECORD_PHOTO_FILES_FALLBACK_ACCEPT} multiple disabled={photoDisabled || selected.length > 0} onChange={(event) => void choose(event.target.files)} /><button className={styles.addButton} type="button" disabled={busy || photoDisabled || selected.length > 0} onClick={openPhotoPicker}>{!backendChecked ? "写真機能を確認中…" : selected.length ? "未完了の写真を先に再試行" : busy ? "圧縮・アップロード中…" : "写真を追加"}</button></> : null}
    {error ? <p className={styles.error} role="alert">{error}</p> : null}
    {diagnostic ? <details className={styles.diagnostic}><summary>エラー詳細</summary>
