...
export function EnvironmentPanel({
  ...
  const dayRows = useMemo(
    () => rows.filter((row) => row.forecastTime.startsWith(selectedDate)),
    [rows, selectedDate],
  );
  const selectedIndex = selectedRow
    ? rows.findIndex((row) => row.forecastTime === selectedRow.forecastTime)
    : -1;
  ...
  const tideEvents = useMemo(
    () => getTideEventsForDate(rows, selectedDate),
    [rows, selectedDate],
  );
  const selectedTideBadge = selectedRow
    ? getTideEventBadgeForForecastTime(
        tideEvents,
        selectedRow.forecastTime,
        selectedDate,
      )
    : null;
  ...
  return (
    <aside className="environmentPanel" aria-live="polite">
      ...
      <div className="environmentSpotFilter">
        <label className="sortSelectLabel" htmlFor="environment-spot">
          地点フィルタ
        </label>
        <select
          id="environment-spot"
        ...
        {selectedSpot ? (
          <p className="environmentSpotMeta">
            {selectedSpot.name}（緯度 {selectedSpot.latitude.toFixed(3)} / 経度{" "}
            {selectedSpot.longitude.toFixed(3)}
      ...
      {!selectedSpot ? (
        <p className="environmentState">
          表示中の釣果地点がないため、環境データの取得対象がありません。
        </p>
      ) : null}
      {isLoading ? (
        <p className="environmentState">
      ...
      {error ? (
        <p className="environmentState error">
          環境データを取得できませんでした。釣果一覧と地図はそのまま利用できます。
        </p>
      ) : null}
      {environment?.warning ? (
        <p className="environmentState warning">{environment.warning}</p>
      ) : null}
      ...
      {environment && selectedRow ? (
        <div className="environmentContentWide">
          <div className="environmentStatusGrid">
            ...
            <span>
              予報対象時刻: {formatDateTime(selectedRow.forecastTime)}
            </span>
            <span>
              データ取得時刻: {formatIsoDateTime(environment.fetchedAt)}
            </span>
          </div>
          ...
          <div className="environmentTimeControls">
            ...
            <button
              ...
              disabled={
                selectedIndex === -1 || selectedIndex >= rows.length - 1
              }
            >
              次の1時間
            </button>
          ...
          <div className="environmentContent">
            <MetricGroup
              ...
              items={[
                [
                  "気温",
                  formatValue(selectedRow.weather?.temperatureCelsius, "℃"),
                ],
                ["天気", selectedRow.weather?.weatherLabel ?? "--"],
                [
                  "降水量",
                  formatValue(selectedRow.weather?.precipitationMm, "mm"),
                ],
                [
                  "降水確率",
                  formatValue(
                    selectedRow.weather?.precipitationProbabilityPercent,
                    "%",
                  ),
                ],
                [
                  "風速",
                  formatValue(selectedRow.weather?.windSpeedKmh, "km/h"),
                ],
                [
                  "風向",
                  formatDirection(
                    selectedRow.weather?.windDirectionLabel,
                    selectedRow.weather?.windDirectionDegrees,
                  ),
                ],
            ...
            <MetricGroup
              ...
              items={[
                [
                  "海面水温",
                  formatValue(
                    selectedRow.marine?.seaSurfaceTemperatureCelsius,
                    "℃",
                  ),
                ],
                [
                  "潮位参考値",
                  formatValue(selectedRow.marine?.seaLevelHeightMslMeters, "m"),
                ],
                [
                  "波高",
                  formatValue(selectedRow.marine?.waveHeightMeters, "m"),
                ],
                [
                  "波向",
                  formatDirection(
                    selectedRow.marine?.waveDirectionLabel,
                    selectedRow.marine?.waveDirectionDegrees,
                  ),
                ],
                [
                  "波周期",
                  formatValue(selectedRow.marine?.wavePeriodSeconds, "秒"),
                ],
                [
                  "海流速度",
                  formatValue(
                    selectedRow.marine?.oceanCurrentVelocityKmh,
                    "km/h",
                  ),
                ],
                [
                  "海流方向",
                  formatDirection(
                    selectedRow.marine?.oceanCurrentDirectionLabel,
                    selectedRow.marine?.oceanCurrentDirectionDegrees,
                  ),
                ],
              ]}
            />
...
function TideReferenceBlock({
  ...
  return (
    <div className="tideReference">
      ...
      <span>
        選択日の潮回り参考:{" "}
        <strong className="tidePhaseName">{tidePhase ?? "--"}</strong>
        （月齢からの簡易算出）
      </span>
      <span>満潮・干潮参考: {formatTideEvents(tideEvents)}</span>
      <span>
        {environment.tideReference.referenceName
          ? `気象庁参照候補: ${environment.tideReference.referenceName}`
          : "気象庁参照地点: 未設定"}
      </span>
      <a
        href={environment.tideReference.url}
        target="_blank"
...
function HourlyList({
  ...
  return (
    <div className="hourlyScroller" aria-label="選択日の時間別一覧">
      {rows.map((row) => {
        const tideBadge = getTideEventBadgeForForecastTime(
          tideEvents,
          row.forecastTime,
          row.forecastTime.slice(0, 10),
        );
        return (
          <button
            type="button"
            key={row.forecastTime}
            className={
              row.forecastTime === selectedTime
                ? "hourlyCard active"
                : "hourlyCard"
            }
            onClick={() => onSelect(row.forecastTime)}
          >
            <strong>{row.forecastTime.slice(11, 16)}</strong>
            {tideBadge ? (
              <span className={`tideEventBadge ${tideBadge.type}`}>
                {tideBadge.label}
              </span>
            ) : null}
            ...
            <span>
              水温 {formatValue(row.marine?.seaSurfaceTemperatureCelsius, "℃")}
            </span>
          </button>
        );
      })}
...
function MetricGroup({
  ...
  title: string;
  items: [string, string][];
  badge?: { type: "high" | "low"; label: string } | null;
}) {
  return (
    <div className="environmentGroup">
      <h3>
        {title}
        {badge ? (
          <span className={`tideEventBadge ${badge.type}`}>{badge.label}</span>
        ) : null}
      </h3>
...
function getDataStatusLabel(environment: FishingEnvironment | null) {
  ...
  const cache =
    environment.cacheStatus === "fresh"
      ? "最新"
      : environment.cacheStatus === "cache-fresh"
        ? "キャッシュ"
        : environment.cacheStatus === "cache-stale"
          ? "古いキャッシュ"
          : "キャッシュなし";
  const partial =
    availability === "weather-only"
      ? " / 天気のみ"
      : availability === "marine-only"
        ? " / 海況のみ"
        : "";
...
function formatTideEvents(events: TideEvent[]) {
  ...
  return events
    .map(
      (event) =>
        `${event.type === "high" ? "満潮" : "干潮"} ${event.approximateTime.slice(0, 2)}時ごろ`,
    )
    .join(" / ");
}
...
function formatDateTime(value: string) {
  return new Intl.DateTimeFormat("ja-JP", {
    ...
    timeStyle: "short",
    timeZone: "Asia/Tokyo",
  }).format(parseForecastTime(value));
}
