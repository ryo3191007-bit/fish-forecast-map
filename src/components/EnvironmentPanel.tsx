...
export function EnvironmentPanel({
  ...
  const selectedTideBadge = selectedRow ? getTideEventBadgeForForecastTime(tideEvents, selectedRow.forecastTime, selectedDate) : null;
    ...
    <aside className="environmentPanel" aria-live="polite">
      ...
      {environment && selectedRow ? (
        <div className="environmentContentWide">
          ...
          <HourlyList tideEvents={tideEvents} rows={dayRows} selectedTime={selectedRow.forecastTime} onSelect={setSelectedTime} />
        </div>
      ) : !isLoading && !error ? (
...
function HourlyList({ rows, selectedTime, onSelect, tideEvents }: { rows: EnvironmentForecastRow[]; selectedTime: string; onSelect: (time: string) => void; tideEvents: TideEvent[] }) {
  return (
    <div className="hourlyScroller" aria-label="選択日の時間別一覧">
      {rows.map((row) => {
        const tideBadge = getTideEventBadgeForForecastTime(tideEvents, row.forecastTime, row.forecastTime.slice(0, 10));
        return (
        <button type="button" key={row.forecastTime} className={row.forecastTime === selectedTime ? "hourlyCard active" : "hourlyCard"} onClick={() => onSelect(row.forecastTime)}>
          ...
          {tideBadge ? <span className={`tideEventBadge ${tideBadge.type}`}>{tideBadge.label}</span> : null}
...
function MetricGroup({ title, items, badge }: { title: string; items: [string, string][]; badge?: { type: "high" | "low"; label: string } | null }) {
  return (
    <div className="environmentGroup">
      <h3>{title}{badge ? <span className={`tideEventBadge ${badge.type}`}>{badge.label}</span> : null}</h3>
