...
export const BATHYMETRY_FALLBACK_HILLSHADE_LAYER_ID =
  "bathymetry-fallback-hillshade";
export const BATHYMETRY_FALLBACK_CONTOUR_LAYER_ID =
...
export const BATHYMETRY_FALLBACK_LICENSE_NOTE = `${BATHYMETRY_FALLBACK_RESOLUTION} / NOAA NCEI ETOPO 2022 / CC0-1.0`;
...
export const BATHYMETRY_VIEW_PRESETS = [
  ...
  { id: "oblique", label: "斜め", pitch: 52, bearing: -18 },
  {
    id: "low",
    label: "低角度",
    pitch: BATHYMETRY_LOW_ANGLE_PITCH,
    bearing: -18,
  },
...
export function normalizeBathymetryExaggeration(value: number) {
  ...
  return Number(
    (
      ...
      steps * BATHYMETRY_EXAGGERATION_STEP
    ).toFixed(2),
  );
}
...
export function classifyTidCode(code: number): TidCategory {
  ...
  if (
    (TID_CLASSIFICATION.predictedInterpolated as readonly number[]).includes(
      code,
    )
  ) {
