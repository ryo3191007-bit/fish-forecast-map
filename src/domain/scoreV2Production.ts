...
const METHOD_AFFINITY: Record<FishingMethod, Record<SupportedSpecies, number | null>> = {
  ジギング: { マアジ: 60, スズキ: 80, チヌ: null }, キャスティング: { マアジ: 80, スズキ: 100, チヌ: 80 },
  コマセ: { マアジ: 100, スズキ: null, チヌ: 100 }, 泳がせ: { マアジ: null, スズキ: 100, チヌ: null },
  サビキ: { マアジ: 100, スズキ: null, チヌ: null }, エギング: { マアジ: null, スズキ: null, チヌ: null }, その他: { マアジ: null, スズキ: null, チヌ: null },
};
...
const habitatScores: Record<SupportedSpecies, Record<string, number>> = {
  ...
  スズキ: { estuary:100, brackish:100, river_influence:100, seaweed_bed:100, beach:100, inner_bay:90, fishing_port:90, sand_mud:90, structure:80, open_sea:60, rocky:60 },
...
const temperatureScore = (species:SupportedSpecies,t:number) => {
  ...
  if (species==="スズキ") return t<6?20:t<12?40:t<17?60:t<21?80:t<=27?100:t<30?80:t<33?40:20;
  return t<10?20:t<15?40:t<20?60:t<23?80:t<=29?100:t<=32?60:t<=35?40:20;
};
const tideScores:Record<SupportedSpecies,Record<TideState,number>>={ マアジ:{上げ潮:80,満潮前後:70,下げ潮:60,干潮前後:60}, スズキ:{上げ潮:80,満潮前後:70,下げ潮:70,干潮前後:60}, チヌ:{上げ潮:80,満潮前後:80,下げ潮:70,干潮前後:60} };
...
function environmentEvidence(environment:FishingEnvironment|null,selected:string,species:SupportedSpecies) {
  ...
  const date=selected.slice(0,10),sun=environment.dailySun.find((d)=>d.date===date); let timeScore:number|null=null; if(sun){const t=new Date(selected).getTime(),rise=new Date(sun.sunrise).getTime(),set=new Date(sun.sunset).getTime(); const band=3600000; const period=Math.abs(t-rise)<=band?"朝":Math.abs(t-set)<=band?"夕":t>rise&&t<set?"昼":"夜"; timeScore=({マアジ:{朝:100,昼:70,夕:100,夜:90},スズキ:{朝:90,昼:70,夕:100,夜:90},チヌ:{朝:90,昼:90,夕:90,夜:70}} as const)[species][period];}
