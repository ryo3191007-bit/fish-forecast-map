...
export function getTideEventsForDate(rows: EnvironmentForecastRow[], dateText: string): TideEvent[] {
  const tideRows = rows
    .filter((row) => row.forecastTime.startsWith(dateText))
    .map((row) => ({ time: row.forecastTime, height: row.marine?.seaLevelHeightMslMeters }))
    .filter((row): row is { time: string; height: number } => typeof row.height === "number");

  if (tideRows.length < 3) return [];

  const events: TideEvent[] = [];
  for (let index = 1; index < tideRows.length - 1; index += 1) {
