...
const CACHE_VERSION = "environment-forecast-v2";
...
export function readCachedFishingEnvironment(point: EnvironmentPoint, storage: StorageLike | null = getLocalStorage()): FishingEnvironment | null {
  ...
  try {
    ...
    if (parsed.version !== CACHE_VERSION || typeof parsed.savedAt !== "string" || !isFishingEnvironmentLike(parsed.environment)) {
      return null;
    }
...
export async function fetchLatestFishingEnvironment(point: EnvironmentPoint, signal?: AbortSignal): Promise<FishingEnvironment> {
  const [weatherResult, marineResult] = await Promise.allSettled([
    fetchOpenMeteoWeatherRows(point, signal),
    fetchOpenMeteoMarineRows(point, signal),
  ]);
...
export async function fetchOpenMeteoWeatherRows(point: EnvironmentPoint, signal?: AbortSignal): Promise<EnvironmentForecastRow[]> {
  ...
  return times.map((time, index) => {
    ...
    const weather: WeatherEnvironment = {
      temperatureCelsius: getNumberAt(data.hourly?.temperature_2m, index),
      weatherCode,
      weatherLabel: getWeatherCodeLabel(weatherCode),
      precipitationMm: getNumberAt(data.hourly?.precipitation, index),
      precipitationProbabilityPercent: getNumberAt(data.hourly?.precipitation_probability, index),
      windSpeedKmh: getNumberAt(data.hourly?.wind_speed_10m, index),
      windDirectionDegrees,
      windDirectionLabel: getCompassDirection(windDirectionDegrees),
      windGustKmh: getNumberAt(data.hourly?.wind_gusts_10m, index),
      observedAt: time,
    };
...
export async function fetchOpenMeteoMarineRows(point: EnvironmentPoint, signal?: AbortSignal): Promise<EnvironmentForecastRow[]> {
  ...
  return times.map((time, index) => {
    ...
    const marine: MarineEnvironment = {
      seaSurfaceTemperatureCelsius: getNumberAt(data.hourly?.sea_surface_temperature, index),
      seaLevelHeightMslMeters: getNumberAt(data.hourly?.sea_level_height_msl, index),
      waveHeightMeters: getNumberAt(data.hourly?.wave_height, index),
      waveDirectionDegrees,
      waveDirectionLabel: getCompassDirection(waveDirectionDegrees),
      wavePeriodSeconds: getNumberAt(data.hourly?.wave_period, index),
      oceanCurrentVelocityKmh: getNumberAt(data.hourly?.ocean_current_velocity, index),
...
function combineAbortSignals(signal: AbortSignal | undefined, timeoutSignal: AbortSignal) {
...
function getTimeArray(value: unknown) {
...
function isFishingEnvironmentLike(value: unknown): value is FishingEnvironment {
  ...
  return (
    ...
    environment.hourly.every(isEnvironmentForecastRowLike) &&
    typeof environment.fetchedAt === "string" &&
    typeof environment.weatherAvailable === "boolean" &&
    typeof environment.marineAvailable === "boolean" &&
...
function isEnvironmentForecastRowLike(row: unknown): row is EnvironmentForecastRow {
  return typeof row === "object" && row !== null && typeof (row as { forecastTime?: unknown }).forecastTime === "string";
}
