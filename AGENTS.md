...
MVPでは機能を小さく保ち、低コストで安全に検証できる範囲に限定します。
...
- Post-MVPでは、既存の外部釣果メモ構造を利用した手入力釣果登録とSupabase Auth/DB保存が実装済みです。
- 外部の釣果サイトの自動取り込みは実装しません。
- 3D海底地形表示は実装しません。
- 有料APIは実装しません。
- 複雑な機械学習による予測は実装しません。
...
- 釣果一覧は手入力釣果のみ表示し、モック釣果は地図・地点評価・既存SCORE用の参考データとして残します。
- 認証UIはヘッダー操作のモーダルです。
- 画面下部の外部サイトリンクは参考閲覧用で、自動取り込み、スクレイピング、定期アクセス、AI解析は行いません。
