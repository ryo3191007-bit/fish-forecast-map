...
assert.match(map, /className=\{`currentLocationMapButton/, "the location icon remains in the map action column");
assert.match(map, /title="現在地を表示"/);
assert.match(mapStyles, /\.currentLocationMapButton svg \{ width:20px; height:20px;/, "location icon retains its compact visual size");
