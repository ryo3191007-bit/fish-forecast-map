...
const details = JSON.parse(fs.readFileSync("data/curation/fishing-spots/issue-250-detail-curation.json", "utf8")) as Details;
const master = fishingSpots;
const seed = fs.readFileSync("supabase/sql/003_master_data_seed.sql", "utf8");
const migration = fs.readFileSync("supabase/migrations/20260722233000_add_issue_250_matsuura_spots.sql", "utf8");
...
for (const candidate of audit.activeCandidates) {
  ...
  assert.equal(candidate.relationToExisting.existingSpotId, expected.broad);
  assert.deepEqual(JMA_AREA_BY_SPOT[candidate.spotId], { prefectureEntryCode: "420000", municipalityCode: "4220800", areaName: "長崎県松浦市" });
...
for (const legacyId of ["fukushima-area", "takashima-area"]) {
  ...
  assert.match(seed, new RegExp(`\\('${legacyId}',[\\s\\S]*?, true\\)`));
}
assert.doesNotMatch(migration, /delete\s+from|drop\s+(table|column)|update\s+public\.fishing_spots/i);
assert.match(migration, /on conflict \(id\) do nothing/);
assert.ok(fallback.includes("issue-250-detail-curation.json"));
...
assert.ok(audit.activeCandidates.some(({ relationToExisting }) => relationToExisting.existingSpotId === "fukushima-area"));
assert.ok(audit.activeCandidates.some(({ relationToExisting }) => relationToExisting.existingSpotId === "takashima-area"));
console.log("Issue #250 Matsuura spot tests passed");
