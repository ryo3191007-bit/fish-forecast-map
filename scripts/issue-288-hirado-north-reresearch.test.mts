...
const files = [
  "data/curation/fishing-spots/issue-288-hirado-seto-reresearch.json",
  "data/curation/fishing-spots/issue-288-hirado-north-reresearch.json",
];
...
assert.ok(curations.every(({ issue }) => issue === 288));
...
for (const curation of curations) {
  ...
  assert.match(curation.researchPolicy.scopeGuard, /適用範囲|全域|混同|広げず/);
  assert.match(curation.researchPolicy.restrictionGuard, /注意|現地掲示/);
}
...
for (const spot of spots) {
  ...
  for (const value of spot.values) {
    ...
    assert.equal(value.id.endsWith(":issue288"), true, `${spot.spotId}:${value.itemKey}: override suffix required`);
...
const hiradoSetoRaw = spots.find(({ spotId }) => spotId === "hirado-seto")!;
for (const value of hiradoSetoRaw.values.filter(({ itemKey }) => ["coastal_topography", "spot_features", "shore_access", "restriction_status", "fishable_area"].includes(itemKey))) {
  assert.match(value.note ?? "", /水道|全域|個別|周辺/, `hirado-seto:${value.itemKey}: broad-area scope must be explicit`);
}
const tabiraRaw = spots.find(({ spotId }) => spotId === "tabira-port")!;
assert.match(tabiraRaw.values.find(({ itemKey }) => itemKey === "restriction_status")?.valueText ?? "", /釣り禁止/);
const tasukeRaw = spots.find(({ spotId }) => spotId === "tasuke-fishing-port")!;
assert.match(tasukeRaw.values.find(({ itemKey }) => itemKey === "obstacles")?.valueTextList.join("、") ?? "", /水没/);
assert.match(tasukeRaw.values.find(({ itemKey }) => itemKey === "restriction_status")?.valueText ?? "", /水没/);
...
for (const spotId of TARGET_SPOTS) {
  ...
  for (const itemKey of expectedKeys) {
    ...
    assert.equal(actual.id.endsWith(":issue288"), true);
...
const tabira = detailSet("tabira-port");
assert.deepEqual(formatTerrainDetailForPresentation(tabira, "spot_features"), { text: "堤防、岸壁", confidence: "low" });
assert.equal(formatSpotDetailValue(findDisplayableSpotDetail(tabira, "parking")), "調査済み・未確定");
assert.match(formatSpotDetailValue(findDisplayableSpotDetail(tabira, "restriction_status")), /釣り禁止/);

const hiradoPort = detailSet("hirado-port");
assert.equal(formatSpotDetailValue(findDisplayableSpotDetail(hiradoPort, "spot_features")), "桟橋");
assert.equal(formatSpotDetailValue(findDisplayableSpotDetail(hiradoPort, "parking")), "平戸港交流広場駐車場");

const hiradoSeto = detailSet("hirado-seto");
assert.match(formatSpotDetailValue(findDisplayableSpotDetail(hiradoSeto, "coastal_topography")), /狭水道/);
assert.equal(formatSpotDetailValue(findDisplayableSpotDetail(hiradoSeto, "tidal_flow")), "調査済み・未確定");

const tasuke = detailSet("tasuke-fishing-port");
assert.deepEqual(formatTerrainDetailForPresentation(tasuke, "spot_features"), { text: "堤防、テトラ、磯", confidence: "low" });
assert.match(formatSpotDetailValue(findDisplayableSpotDetail(tasuke, "restriction_status")), /水没/);

const usukawan = detailSet("usukawan-fishing-port");
assert.deepEqual(formatTerrainDetailForPresentation(usukawan, "spot_features"), { text: "堤防、岸壁、テトラ", confidence: "low" });
assert.equal(formatSpotDetailValue(findDisplayableSpotDetail(usukawan, "toilet")), "調査済み・未確定");

const hoki = detailSet("hoki-fishing-port");
assert.deepEqual(formatTerrainDetailForPresentation(hoki, "spot_features"), { text: "堤防", confidence: "low" });

const override = tabira.values.find((value) => value.itemKey === "spot_features")!;
const oldCurated = {
  ...override,
...
assert.match(fallbackSource, /issue288SetoDetails/);
assert.match(fallbackSource, /issue288NorthDetails/);
...
console.log("Issue #288 Hirado Seto and north spot re-research tests passed");
