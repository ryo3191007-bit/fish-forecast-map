...
const full = scoreV2.calculateScoreV2ForSpecies({ species:'アジ', spot, selectedDateTime:'2026-07-18T06:00', spotEvidence:evidence, environmentEvidence:approvedEnvironment });
...
const legacyOnly = scoreV2.calculateScoreV2ForSpecies({ species:'アジ', spot, selectedDateTime:'2026-07-18T06:00' });
...
const rawEnvironmentOnly = scoreV2.calculateScoreV2ForSpecies({ species:'アジ', spot, selectedDateTime:'2026-07-18T06:00', spotEvidence:evidence });
...
const boundary = scoreV2.calculateScoreV2ForSpecies({ species:'アジ', spot, selectedDateTime:'2026-07-18T06:00', spotEvidence:{ directSpecies:evidence.directSpecies, habitat:null, catchHistory:evidence.catchHistory, methodAffinity:null }, environmentEvidence:{ waterTemperature:approvedEnvironment.waterTemperature, tideCurrent:null, windWave:approvedEnvironment.windWave, seasonTime:approvedEnvironment.seasonTime, weatherRain:null } });
...
const missingEnvironment = scoreV2.calculateScoreV2ForSpecies({ species:'アジ', spot, selectedDateTime:'2026-07-18T06:00', spotEvidence:evidence, environmentEvidence:{ waterTemperature:null, tideCurrent:null, windWave:null, seasonTime:null, weatherRain:null } });
...
const under = scoreV2.calculateScoreV2ForSpecies({ species:'アジ', spot, selectedDateTime:'2026-07-18T06:00', spotEvidence:{ directSpecies:evidence.directSpecies, habitat:null, catchHistory:null, methodAffinity:null } });
...
for (const species of ['アジ', 'シーバス', 'チヌ']) {
  const result = scoreV2.calculateScoreV2ForSpecies({ species, spot:{...spot, targetSpecies:[species], recommendedMethods: species === 'シーバス' ? ['キャスティング'] : ['コマセ']}, selectedDateTime:'2026-07-18T06:00', spotEvidence:evidence, environmentEvidence:approvedEnvironment });
