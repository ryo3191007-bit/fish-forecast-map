...
for (const species of ['マアジ', 'シーバス', 'チヌ']) {
...
const speciesResults = [90,80,70,60].map((overallScore, i) => ({ species:['マアジ','シーバス','チヌ','サバ'][i], overallScore }));
...
const methodThree = scoreV2.calculateScoreV2ForMethod('サビキ', speciesResults, { 'サビキ':['マアジ','シーバス','チヌ'] }, suitability);
const methodFour = scoreV2.calculateScoreV2ForMethod('サビキ', speciesResults, { 'サビキ':['マアジ','シーバス','チヌ','サバ'] }, suitability);
...
assert(methodThree.contributingSpecies.join(',') === 'マアジ,シーバス,チヌ' && methodThree.speciesAverageScore === 80 && methodThree.overallScore === 86, 'three compatible species should average all three');
assert(methodFour.contributingSpecies.join(',') === 'マアジ,シーバス,チヌ' && methodFour.speciesAverageScore === 80, 'more than three compatible species should use only top three scores');
