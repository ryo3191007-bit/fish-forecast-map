...
const checks = [
  ...
  ["delete is logical update", /\.update\(\{ is_deleted: true, updated_at: new Date\(\)\.toISOString\(\) \}\)/.test(repository)],
  ["delete scopes by id and owner", /\.eq\("id", memoId\)[\s\S]*\.eq\("owner_id", clientStatus\.userId\)/.test(repository)],
