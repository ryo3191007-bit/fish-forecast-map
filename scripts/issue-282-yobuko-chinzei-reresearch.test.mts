...
const files = [
  "data/curation/fishing-spots/issue-282-yobuko-chinzei-reresearch-yobuko.json",
  "data/curation/fishing-spots/issue-282-yobuko-chinzei-reresearch-chinzei.json",
];
...
assert.ok(curations.every(({ issue }) => issue === 282));
assert.deepEqual(new Set(spots.map(({ spotId }) => spotId)), new Set(TARGET_SPOTS));
assert.equal(spots.flatMap(({ values }) => values).length, 39);
...
for (const spot of spots) {
  ...
  for (const value of spot.values) {
    ...
    assert.equal(value.id.endsWith(":issue282"), true, `${spot.spotId}:${value.itemKey}: override suffix required`);
...
const yobukoAreaRaw = spots.find(({ spotId }) => spotId === "yobuko-area")!;
for (const value of yobukoAreaRaw.values.filter(({ itemKey }) => ["coastal_topography", "spot_features", "shore_access"].includes(itemKey))) {
  assert.match(value.note ?? "", /部分|全域/, `yobuko-area:${value.itemKey}: broad-area scope must be limited`);
}
...
for (const spotId of TARGET_SPOTS) {
  ...
  for (const itemKey of expectedKeys) {
    ...
    assert.equal(actual.id.endsWith(":issue282"), true);
...
const yobukoArea: FishingSpotDetailSet = {
...
const kodomo: FishingSpotDetailSet = {
  itemDefinitions: runtime.itemDefinitions,
...
const hado: FishingSpotDetailSet = {
  itemDefinitions: runtime.itemDefinitions,
...
const nagoya: FishingSpotDetailSet = {
  itemDefinitions: runtime.itemDefinitions,
...
assert.match(fallbackSource, /issue282YobukoDetails/);
assert.match(fallbackSource, /issue282ChinzeiDetails/);
...
console.log("Issue #282 Yobuko Chinzei spot re-research tests passed");
