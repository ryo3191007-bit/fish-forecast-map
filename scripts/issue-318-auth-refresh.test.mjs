...
const checks = [
  ...
  ["Google OAuth is implemented", authHook.includes("signInWithOAuth") && authHook.includes('provider: "google"')],
  ...
  ["password recovery event is handled", authHook.includes('event === "PASSWORD_RECOVERY"') || authHook.includes("isRecoveryEvent(event)")],
  ["legacy magic-link sign in is removed", !authHook.includes("signInWithOtp")],
  ["Google login is visible", authPanel.includes("Googleで続ける")],
  ...
  ["Google provider setup is documented", setupDoc.includes("Google") && setupDoc.includes("Authorized redirect URI")],
