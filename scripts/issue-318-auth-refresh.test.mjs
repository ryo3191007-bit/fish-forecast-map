...
const checks = [
  ...
  ["email rate limit error is localized", authHook.includes("email rate limit") && authHook.includes("認証メールの送信回数が上限に達しました")],
  ...
  ["custom SMTP is documented", setupDoc.includes("Custom SMTP") && setupDoc.includes("Built-in email provider")],
