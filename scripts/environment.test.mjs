...
await test('満潮バッジ情報を同一日同一時刻へ対応づける', async () => {
  const badge = env.getTideEventBadgeForForecastTime('2026-07-11T10:30', [{ type: 'high', approximateTime: '10:00', heightMeters: 2 }], '2026-07-11');
  assert.deepEqual(badge, { type: 'high', label: '満潮' });
});
await test('干潮バッジ情報を同一日同一時刻へ対応づける', async () => {
  const badge = env.getTideEventBadgeForForecastTime('2026-07-11T04:15', [{ type: 'low', approximateTime: '04:00', heightMeters: 0.1 }], '2026-07-11');
  assert.deepEqual(badge, { type: 'low', label: '干潮' });
});
await test('非該当時刻・別日・潮位イベントなしはバッジなし', async () => {
  const events = [{ type: 'high', approximateTime: '10:00', heightMeters: 2 }];
  assert.equal(env.getTideEventBadgeForForecastTime('2026-07-11T09:00', events, '2026-07-11'), null);
  assert.equal(env.getTideEventBadgeForForecastTime('2026-07-12T10:00', events, '2026-07-11'), null);
  assert.equal(env.getTideEventBadgeForForecastTime('2026-07-11T10:00', [], '2026-07-11'), null);
});
