...
assert.match(css, /@media \(max-width: 620px\)\s*\{[\s\S]*?\.mapSection\s*\{\s*margin-inline: -16px;[\s\S]*?\.map\s*\{\s*height: 580px;/);
assert.match(css, /@media \(max-width: 420px\)\s*\{[\s\S]*?\.mapSection\s*\{\s*margin-inline: -12px;[\s\S]*?\.map\s*\{\s*height: 580px;/);
