...
assert.ok(fishingSpots.length>=52); assert.equal(new Set(fishingSpots.map(s=>s.id)).size,fishingSpots.length); assert.deepEqual(new Set(audit.activeCandidates.map((x)=>x.spotId)),new Set(EXPECTED.keys()));
...
assert.ok(fallback.includes("issue292HiradoSouthDetails"),"later re-research may supersede Issue #253 runtime values for the final southern five spots");
