...
assert.equal(fishingSpots.length, 52, "Issue #253 adds eight spots after Issue #254");
...
assert.equal(fishingSpots.filter(({ id }) => EXPECTED.has(id)).length, 4); assert.equal(new Set(fishingSpots.map(({ id }) => id)).size, 52);
