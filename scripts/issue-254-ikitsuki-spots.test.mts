...
assert.ok(fishingSpots.length >= 52, "the Issue #253/#254 spot set remains in the growing master");
...
assert.equal(fishingSpots.filter(({ id }) => EXPECTED.has(id)).length, 4); assert.equal(new Set(fishingSpots.map(({ id }) => id)).size, fishingSpots.length);
